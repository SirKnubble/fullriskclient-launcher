...
import type { ChatMessage, MessageReaction } from "../../types/messaging";
import { useMinecraftAuthStore } from "../../store/minecraft-auth-store";
import { useFriendsStore } from "../../store/useFriendsStore";
import {
  useMessagingStore,
  markChatAsGloballyRead,
} from "../../store/useMessagingStore";
import { useThemeStore } from "../../store/useThemeStore";
import { useReverseSequentialLoading } from "../../hooks/useReverseSequentialLoading";
...
export default function ChatWindow({
  ...
  useEffect(() => {
    const loadInitialMessages = async () => {
      ...
      const existingMessages =
        useMessagingStore.getState().messages[chatId] || [];
  ...
  const handleReaction = async (messageId: string, emoji: string) => {
    try {
      ...
      const currentUserId = friendsUserId || authUserId || authUsername;
      if (!currentUserId) {
      ...
      const existingReaction = currentReactions.find((r) => {
        const userId = r.userId || r.reactor;
        const reactionEmoji = r.emoji || r.emojiUnicode;
        return userId === currentUserId && reactionEmoji === emoji;
      });
      ...
      updateMessage(chatId, optimisticMessage);

      try {
        let updatedMessage;

        if (existingReaction) {
          updatedMessage = await removeReaction(messageId);
        } else {
          updatedMessage = await reactToMessage({ messageId, emoji });
        }

        if (updatedMessage && updatedMessage.reactions !== undefined) {
          updateMessage(chatId, updatedMessage);
        }
      } catch (error) {
  ...
  const isOwnMessage = useCallback(
    ...
    [currentUser, friendsCurrentUser]
  );
    ...
    (reactions: MessageReaction[] = []) => {
      const grouped = reactions
        .filter(
          (reaction) =>
            reaction &&
            (reaction.emoji || reaction.emojiUnicode) &&
            (reaction.userId || reaction.reactor)
        )
        .reduce(
          (acc, reaction) => {
            const emoji = reaction.emoji || reaction.emojiUnicode;
            const userId = reaction.userId || reaction.reactor;
  ...
  const messagesLoader = useReverseSequentialLoading(messages, {
    itemsPerPage: 10,
    initialLoadCount: 20,
    loadThreshold: 5,
    enabled: messages.length > 10,
  });
  ...
  const groupedMessages = React.useMemo(() => {
    ...
    const displayedMessages = messagesLoader.displayedItems;

    displayedMessages.forEach((message) => {
      const messageDate = new Date(message.createdAt).toDateString();
      const isNewDay = messageDate !== lastDate;
      const isNewSender =
        !currentGroup || currentGroup.senderId !== message.senderId;
      const timeDiff = currentGroup
        ? message.createdAt - currentGroup.timestamp
        : 0;
    ...
    return groups;
  }, [messagesLoader.displayedItems, getSenderDisplayName]);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-4 border-b border-white/10">
        <IconButton
        ...
        <Avatar
          ...
          showSkeleton={true}
        />
        <span className="font-minecraft text-white text-3xl sm:text-4xl font-medium truncate">
          {recipientName}
        </span>
      </div>
      ...
      <div className="flex-1 overflow-hidden">
        <div
          ...
          className="h-full overflow-y-auto p-2 sm:p-4 space-y-3 sm:space-y-4 custom-scrollbar"
          style={{ minWidth: 0 }}
          onClick={() => {
            setShowEmojiPicker(false);
            setSelectedMessageId(null);
        ...
        >
          {messagesLoader.hasMore && (
            <div
              ref={messagesLoader.scrollSentinelRef}
              className="h-4 flex justify-center mb-4"
            >
              {messagesLoader.isLoading && (
            ...
            groupedMessages.map((group, groupIndex) => {
              const showDateSeparator =
                ...
                new Date(group.timestamp).toDateString() !==
                  new Date(
                    groupedMessages[groupIndex - 1].timestamp
                  ).toDateString();
                    ...
                    >
                      ...
                      >
                        <span
                          ...
                          style={{
                            color: isOwnMessage(group.senderId)
                              ? accentColor.value
                              : "white",
                          }}
                        >
                          ...
                          {editingMessageId === message._id ? (
                            ...
                            >
                              <textarea
                                ...
                                className="w-full p-2 font-minecraft-ten text-xs resize-none text-white border-none outline-none bg-black/30 overflow-hidden break-words"
                                style={{
                                  borderRadius: `${borderRadius}px`,
                                  minHeight: "40px",
                                  maxHeight: "120px",
                            ...
                            >
                              <div
                                className={cn(
                                  "p-2 sm:p-3 transition-all duration-200 max-w-[85%] sm:max-w-[70%] md:max-w-[60%] lg:max-w-[50%] relative backdrop-blur-md hover:bg-white/5 min-w-0",
                                  isOwnMessage(message.senderId)
                              ...
                              >
                                <p
                                  className={cn(
                                    "font-minecraft-ten text-xs break-words break-all",
                                    isOwnMessage(message.senderId)
                                      ? "text-white"
                                      : "text-white"
                                  )}
                                    ...
                                    <div className="flex flex-wrap gap-1 mt-2">
                                      {getGroupedReactions(
                                        ...
                                        <button
                                          ...
                                          className={cn(
                                            "inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs transition-colors hover:scale-105 active:scale-95",
                                            groupedReaction.userHasReacted
                                              ? "text-white border-2"
                                        ...
                                        >
                                          <span>{groupedReaction.emoji}</span>
                                          <span className="font-minecraft-ten text-xs font-medium">
        ...
        <div className="flex items-end space-x-2">
          <div className="flex-1 relative">
            <textarea
              ...
              className="w-full p-3 font-minecraft-ten text-xs resize-none text-white border border-white/10 focus:border-white/30 outline-none placeholder-white/40 bg-black/40 overflow-hidden break-words"
              style={{
                borderRadius: `${borderRadius}px`,
                minHeight: "44px",
                maxHeight: "120px",
              ...
              <div className="grid grid-cols-8 gap-2">
                {COMMON_EMOJIS.map((emoji, index) => (
                  <button
                    ...
                    className="w-10 h-10 flex items-center justify-center hover:bg-white/10 transition-all duration-200 text-xl hover:scale-110 active:scale-95"
                    style={{ borderRadius: `${borderRadius}px` }}
                    title={emoji}
                  >
                    {emoji}
